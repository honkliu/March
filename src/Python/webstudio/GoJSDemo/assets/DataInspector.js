...
Inspector.isReadOnly = function(data, propname) {
  if (data instanceof go.Part) data = data.data;
  return  data['type']=='SubGraphSiSo'?false:true;
};
...
Inspector.prototype.inspectObject = function(object) {
  ...
  if (!inspectedObjects || !this.multipleSelection) {
    ...
    for (var name in declaredProperties) {
      ...
      if(['activation', 'keep_dim', 'abs_flag', 'scale', 'batch_norm', 'last_hidden_activation',
          'last_hidden_softmax', 'operation', 'pool_type'].indexOf(name)>=0){
        continue;
      }
      tbody.appendChild(this.buildPropertyRow(name, val, data['necessary']));
    }
    ...
    if (this.includesOwnProperties) {
      for (var k in data) {
        ...
        tbody.appendChild(this.buildPropertyRow(k, data[k], data['necessary']));
      }
    }
...
*/
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue, necessary = []) {
  var mainDiv = this._div;
  var tr = document.createElement("tr");

  var td1 = document.createElement("td");
  if(necessary.indexOf(propertyName) >= 0){
     td1.textContent = propertyName + '(*)';
     td1.style.color = '#a94a4a';
  }else{
     td1.textContent = propertyName;
  }
  tr.appendChild(td1);
